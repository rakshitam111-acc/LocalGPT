...
:: 1. Clear any stuck process on port 8000
echo [1/4] Checking Port 8000...
for /f "tokens=5" %%a in ('netstat -aon ^| findstr :8000 ^| findstr LISTENING') do (
    echo Terminating old process on port 8000 (PID: %%a)...
...
echo [2/4] Starting LocalGPT AI Backend Engine...
start /B "" "%~dp0.venv\Scripts\python.exe" -m uvicorn app.main:app --app-dir "%~dp0backend" --host 0.0.0.0 --port 8000
...
"%~dp0cloudflared.exe" tunnel --no-tls-verify --url http://localhost:8000
